...
class FFTFilter {
    ...
    public function new(buffer: CircularBuffer<Float>, sampleRate: Int, useWindow: Bool=true, doLogScaling: Bool=true) {
        ...
        this.re = HxaArrayUtil.newArray(buffer.length, true, 0.0);
        this.im = HxaArrayUtil.newArray(buffer.length, true, 0.0);
        this.fft = new FFT();
        ...
        this.mag = HxaArrayUtil.newArray(buffer.length >> 1, true, 0.0);
        this.freqs = HxaArrayUtil.newArray(buffer.length >> 1, true, 0.0);
